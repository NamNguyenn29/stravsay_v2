...
import { getPaymentMethods } from "@/api/Payment/getPaymentMethods";
...
export default function CheckoutPage() {
    ...
    const loadPaymentMethods = async () => {
        try {
            ...
            if (res?.isSuccess && Array.isArray(res.list)) {
                setMethods(res.list);
                if (res.list.length > 0 && !paymentMethodId) {
                    setPaymentMethodId(res.list[0].paymentMethodID);
                }
            } else if (Array.isArray(res)) {
                setMethods(res);
                if (res.length > 0 && !paymentMethodId) {
                    setPaymentMethodId(res[0].paymentMethodID);
                }
            } else {
                console.warn("Unexpected payment methods response:", res);
            }
        } catch (err) {
    ...
    const loadServices = async () => {
        try {
            ...
            const res = await serviceServices.getAll(); // GET /Service
            if (res.data && Array.isArray(res.data.list)) {
                setServices(res.data.list); // lấy list từ response
            } else {
                setServices([]);
                console.warn("Unexpected service response:", res.data);
            }
        } catch (err) {
    ...
    // ---------------------------
    // Toggle extra service (multi-select)
    // ---------------------------
    ...
    const onContinue = async () => {
        ...
        try {
            ...
            const amount = 1500000; // TODO: calculate total including services

            const payload = {
                bookingID: bookingId,
                paymentMethodID: chosen.paymentMethodID,
                amount: amount,
                services: selectedServiceIds, // gửi selected services nếu backend hỗ trợ
            };

            const res = await createPayment(payload);

            if (res?.isSuccess) {
                const payUrl = res.string ?? res.object?.payUrl ?? res.object?.responsePayload ?? null;
                if (payUrl && chosen.isOnline) {
                    window.location.href = payUrl;
                    return;
                }
                alert("Payment created successfully. Please follow instructions to complete payment.");
                return;
            }

            alert(res?.message || "Payment creation failed.");
        } catch (err) {
                            ...
                            {loadingServices ? (
                                ...
                                services.map((s) => (
                                    ...
                                    >
                                        ...
                                        <div className="text-sm text-gray-500 mt-1">{s.price.toLocaleString()}₫</div>
                                    </motion.button>
                                ))
                            )}
                        </div>
